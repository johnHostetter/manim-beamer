...
class TestTSK(unittest.TestCase):
    ...
    def test_tsk(self) -> None:
        ...
        antecedents = [
            ...
            Gaussian(
                4,
                centers=torch.tensor([0.2, 0.6, 0.9, 1.2]).float(),
        ...
        assert (
            rule_vertex["type"] == AlgebraicProduct
...
class TestMamdani(unittest.TestCase):
    ...
    def test_mamdani(self) -> None:
        ...
        assert all(
            self.knowledge_base.intra_dimensions(True)
            == np.array([term.in_features for term in self.antecedents])
        )
        assert all(
            self.knowledge_base.intra_dimensions(False)
            == np.array([term.in_features for term in self.consequents])
        )
        ...
        flc = Mamdani(
            out_features=len(self.consequents),
            knowledge_base=self.knowledge_base,
            learning_rate=1e-3,
        )

        # check that the intermediate structure of the Mamdani FLC is correctly built
        input_links, input_offset = self.knowledge_base.matrix(
            AlgebraicProduct, is_input=True
        )
        output_links, output_offset = self.knowledge_base.matrix(
            AlgebraicProduct, is_input=False
        )
        ...
        assert torch.isclose(
            flc.engine.consequences.centers,
            torch.tensor(
                [[0.5000, 0.3000, 0.1000, 0.9000], [-0.2000, -0.7000, -0.9000, 0.0000]]
            ),
        ).all()
        assert torch.isclose(
            flc.engine.consequences.widths,
            torch.tensor(
                [[0.1000, 0.4000, 0.6000, 0.8000], [0.4000, 0.4000, 0.5000, -1.0000]]
            ),
        ).all()
