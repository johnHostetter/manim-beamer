...
class TestECM(unittest.TestCase):
    ...
    def test_general_euclidean_distance(self):
        ...
        distance = general_euclidean_distance(
            torch.tensor(np.array([vector_1])), torch.tensor(vector_2)).float()
    ...
    def test_ecm_output(self):
        ...
        expected_clusters_widths = torch.tensor([
            0.6810045, 0.6918813, 0.6853437, 0.6883185, 0.6896569,
            0.36993876, 0.68684465, 0.67583203, 0.563162, 0., 0.
        ])
        expected_clusters = ContinuousFuzzySet(in_features=clusters.in_features,
