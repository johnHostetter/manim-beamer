...
class TestEDA(unittest.TestCase):
    ...
    def test_frequencies(self):
        ...
        unique_observations, frequencies = np.unique(
            input_train_data, axis=0, return_counts=True
        )
    ...
    def test_reduce_partitioning(self):
        ...
        assert torch.isclose(
            prototypes.centers.detach().float(),
            expected_prototypes_centers.float(),
            atol=1e-1,
        ).all()
        assert torch.isclose(
            prototypes.widths.detach().float(),
            expected_prototypes_widths.float(),
            atol=1e-1,
        ).all()
    ...
    def test_empirical_fuzzy_sets(self):
        ...
        assert torch.isclose(
            empirical_fuzzy_sets.centers.detach().float(),
            expected_prototypes_centers.float(),
            atol=1e-1,
        ).all()
        assert torch.isclose(
            empirical_fuzzy_sets.widths.detach().float(),
            expected_prototypes_widths.float(),
            atol=1e-1,
        ).all()
