...
class TestDependenciesInKnowledgeBase(unittest.TestCase):
    ...
    def test_depends_on(self):
        ...
        assert knowledge_base.indiscernibility(
            {"P", "Q"}
        ) == knowledge_base.indiscernibility({"P"})
        assert (
            knowledge_base.find_restricted_positive_region({"P"}, {"Q"})
            == self.universe
        )
    ...
    def test_partial_depends_on(self):
        ...
        assert knowledge_base.lower_approximation("P", set_x_3) == set_y_3.union(
            set_y_5
        )
        ...
        assert knowledge_base.find_restricted_positive_region(
            "P", "Q"
